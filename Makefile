...
# keychain profile before running make release (see docs/releasing.md).
RELEASE_SIGN_IDENTITY ?= Developer ID Application: Ali (REPLACE_WITH_YOUR_TEAMID)
RELEASE_NOTARY_PROFILE ?= try-ubuntu-ali
...
.DEFAULT_GOAL := help
.PHONY: help doctor test guest runtime app build run run-ephemeral reset package package-preflight release release-preflight clean clean-all clean-guest
...
guest:
	@TRYUBUNTU_FORCE_BUILD="$(FORCE)" "$(BUILD_CACHE)" \
...
app: guest runtime
	@TRYUBUNTU_FORCE_BUILD="$(FORCE)" \
	  TRYUBUNTU_CODESIGN_IDENTITY="$(DEVELOPMENT_SIGN_IDENTITY)" \
	  "$(BUILD_CACHE)" \
...
clean:
	...
	@set -e; if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then \
	  ...
	  done < <(docker container ls -aq --filter ancestor=tryubuntu-guest-builder); \
...
clean-all:
	...
	@user_home=$$(python3 -c 'import os,pwd; print(pwd.getpwuid(os.getuid()).pw_dir)'); \
	  ...
	  cache="$$user_home/Library/Caches/dev.tryubuntu.native"; \
	  preferences="$$user_home/Library/Preferences/dev.tryubuntu.native.plist"; \
	  saved_state="$$user_home/Library/Saved Application State/dev.tryubuntu.native.savedState"; \
	...
	@user_id=$$(id -u); \
	  find /private/tmp -maxdepth 1 -user "$$user_id" \
	    \( -name 'tryubuntu-qemu-source-build.*' \
	       -o -name 'tryubuntu-qemu-gpu-runtime.*' \
	       -o -name 'tryubuntu-qemu-gpu.??????' \
	       -o -name 'tryubuntu-qemu-storage-test.??????' \
	       -o -name 'tryubuntu-dmg.*' \) \
	    -exec rm -rf -- {} +; \
	  ...
	  find "$$user_tmp" -maxdepth 1 -user "$$user_id" \
	    \( -name 'tryubuntu-qemu-request-*' \
	       -o -name 'tryubuntu-qemu-path-*' \
