...
def read_runtime_manifest(path: Path) -> frozenset[str]:
    ...
    for entry in entries:
        if re.fullmatch(r"(?:bin|lib|share)/[A-Za-z0-9][A-Za-z0-9._+-]*", entry) is None:
...
def fingerprint(root: Path, component: str, command: list[str]) -> str:
    digest = hashlib.sha256()
    digest.update(f"try-tryubuntu-build-cache-v{SCHEMA_VERSION}\0{component}\0".encode())
    digest.update(
...
def validate_guest(root: Path, previous: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if (
        ...
        or manifest.get("kind") != "tryubuntu-guest-artifacts"
    ):
...
def validate_runtime(root: Path, previous: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for path in directory.rglob("*"):
        ...
        if path.is_dir():
            if relative not in {"bin", "lib", "share"}:
                raise CacheError(
...
def force_requested(argument: bool) -> bool:
    value = os.environ.get("TRYUBUNTU_FORCE_BUILD", "0").lower()
