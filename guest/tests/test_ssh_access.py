...
class SSHAccessGeneratorTests(unittest.TestCase):
    ...
    def test_absent_and_lookalike_tokens_do_nothing(self) -> None:
        for command_line in (
            "root=/dev/vda rw",
            "root=/dev/vda tryubuntu.ssh_access=0",
            "root=/dev/vda xtryubuntu.ssh_access=1",
            "root=/dev/vda tryubuntu.ssh_access=1x",
        ):
    ...
    def test_exact_token_creates_only_runtime_wants_link(self) -> None:
        result, output, temporary = self.run_generator(
            "root=/dev/vda rw tryubuntu.ssh_access=1"
        )
        with temporary:
    ...
    def test_exact_token_requires_vendor_unit(self) -> None:
        result, output, temporary = self.run_generator(
            "root=/dev/vda rw tryubuntu.ssh_access=1", vendor_unit_exists=False
        )
        with temporary:
