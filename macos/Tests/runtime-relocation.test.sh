...
test_root=$(mktemp -d /private/tmp/tryubuntu-runtime-relocation-test.XXXXXX)
case "$test_root" in
  /private/tmp/tryubuntu-runtime-relocation-test.??????) ;;
...
clang -arch arm64 -mmacosx-version-min=15.0 -dynamiclib \
  ...
  -Wl,-install_name,/private/tryubuntu-build/libfixture.dylib \
  -x c - -o "$runtime/lib/libfixture.dylib" <<'EOF'
int tryubuntu_fixture_value(void) { return 42; }
EOF

for output in "$runtime/bin/fixture" "$missing_runtime/bin/fixture"; do
  clang -arch arm64 -mmacosx-version-min=15.0 \
    -Wl,-headerpad_max_install_names \
    -Wl,-rpath,/private/tryubuntu-build \
    "$runtime/lib/libfixture.dylib" -x c - -o "$output" <<'EOF'
extern int tryubuntu_fixture_value(void);
int main(void) { return tryubuntu_fixture_value() == 42 ? 0 : 1; }
EOF
done
...
[[ $dependencies != *'/private/tryubuntu-build/'* ]] || \
