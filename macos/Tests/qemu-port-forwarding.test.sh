...
assert_rejected() {
  ...
  assert_eq "$QEMU_PORT_FORWARDING_NETDEV" 'user,id=tryubuntu-net'
...
assert_eq \
  ...
  'user,id=tryubuntu-net,hostfwd=tcp:127.0.0.1:8080-:3000'
...
assert_eq \
  "$QEMU_PORT_FORWARDING_NETDEV" \
  'user,id=tryubuntu-net,hostfwd=udp:127.0.0.1:5353-:5353,hostfwd=tcp:127.0.0.1:2222-:22,hostfwd=udp:127.0.0.1:2222-:22'
assert_eq \
...
assert_eq \
  "$QEMU_PORT_FORWARDING_NETDEV" \
  'user,id=tryubuntu-net,hostfwd=udp:127.0.0.1:53-:53'
assert_eq "$QEMU_PORT_FORWARDING_ERROR" ''
assert_eq "$QEMU_PORT_FORWARDING_ENABLES_SSH" 0
