...
# A legacy raw arm64 Image stub: the launcher accepts it with plain direct
# boot, so the fake app needs no bundled UEFI firmware.
fake_kernel() {
...
test_root=$(mktemp -d '/private/tmp/tryubuntu-qemu-ssh-contract.XXXXXX')
case "$test_root" in
  /private/tmp/tryubuntu-qemu-ssh-contract.??????) ;;
...
cat >"$contents/MacOS/tryubuntu-vm-helper" <<'SH'
#!/bin/bash
set -euo pipefail
...
# TRYUBUNTU_SDL_OUTPUT_DEVICE_NAME
...
if is_recovery:
    ...
    for argument in arguments:
        if argument.startswith("local,id=tryubuntu-boot-export,"):
    ...
    recovered = bytearray(64)
    recovered[56:60] = b"ARM\x64"
    export_path.joinpath("kernel").write_bytes(bytes(recovered))
    export_path.joinpath("initramfs").write_bytes(b"070701 recovered-initramfs")
...
QEMU_PERSISTENT_STORAGE_QEMU_ADD_FD='fd=9,set=77,opaque=tryubuntu-persistent-lock'
QEMU_SELECTED_DISK=''
...
fake_kernel "$guest/vmlinuz-linux"
...
assert_line_pair "$test_root/disabled/qemu.log" -netdev 'user,id=tryubuntu-net'
...
assert_not_contains "$disabled_qemu" tryubuntu.ssh_access
...
run_scenario non-immersive 0 '' TRYUBUNTU_QEMU_GPU_IMMERSIVE=0
...
/usr/bin/plutil -replace kernelCommandLine -string \
  ...
  "$guest/launch.plist"
run_scenario enabled 0 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:2223:22
enabled_qemu=$(<"$test_root/enabled/qemu.log")
assert_line_pair "$test_root/enabled/qemu.log" -netdev \
  'user,id=tryubuntu-net,hostfwd=tcp:127.0.0.1:2223-:22'
...
assert_contains "$enabled_qemu" tryubuntu.ssh_access=1
...
grep -aq $'ARM\\x64' "$persistent_root/boot/kernel" || fail "staged boot-kit kernel is not the raw Image stub"
...
run_scenario recovery-failed 81 '' \
  ...
  TRYUBUNTU_QEMU_GPU_ALLOW_BOOT_RECOVERY=1 \
  FAKE_QEMU_STATUS=17
...
assert_line_pair "$test_root/recovery-allowed/recovery.log" -drive \
  "if=none,id=tryubuntu-recovery-root,file=$recovery_root/rootfs.ext4,format=raw,media=disk,cache=none,readonly=on"
assert_line_pair "$test_root/recovery-allowed/recovery.log" -device \
  'virtio-9p-pci,fsdev=tryubuntu-boot-export,mount_tag=try-omarchy-boot-export,romfile='
assert_contains "$recovery_qemu" 'root=/dev/vda ro rootwait'
assert_contains "$recovery_qemu" 'rootflags=noload fsck.mode=skip tryubuntu.export_boot=1'
...
[[ $(grep -o 'tryubuntu.export_boot=1' \
  "$test_root/recovery-allowed/recovery.log" | wc -l | tr -d ' ') == 1 ]] || \
...
grep -aq $'ARM\\x64' "$recovery_root/boot/kernel" || fail "recovered boot-kit kernel is not the raw Image stub"
...
run_scenario preset 0 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:2222:22
assert_line_pair "$test_root/preset/qemu.log" -netdev \
  'user,id=tryubuntu-net,hostfwd=tcp:127.0.0.1:2222-:22'
[[ $(grep -o 'tryubuntu.ssh_access=1' "$test_root/preset/qemu.log" | wc -l | tr -d ' ') == 1 ]] || \
  fail 'preset must append exactly one SSH activation token'

run_scenario udp-22 0 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=udp:2224:22
udp_qemu=$(<"$test_root/udp-22/qemu.log")
assert_contains "$udp_qemu" 'hostfwd=udp:127.0.0.1:2224-:22'
assert_not_contains "$udp_qemu" tryubuntu.ssh_access

run_scenario unrelated 0 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:8080:3000
unrelated_qemu=$(<"$test_root/unrelated/qemu.log")
assert_contains "$unrelated_qemu" 'hostfwd=tcp:127.0.0.1:8080-:3000'
assert_not_contains "$unrelated_qemu" tryubuntu.ssh_access

run_scenario mixed 0 '' \
  'TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=udp:5353:5353;tcp:22022:22;udp:2222:22'
...
[[ $(grep -o 'tryubuntu.ssh_access=1' "$test_root/mixed/qemu.log" | wc -l | tr -d ' ') == 1 ]] || \
  fail 'mixed forwarding must append exactly one SSH activation token'

run_scenario ephemeral 0 --ephemeral TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:2224:22
assert_contains "$(<"$test_root/ephemeral/qemu.log")" \
  'user,id=tryubuntu-net,hostfwd=tcp:127.0.0.1:2224-:22'
assert_contains "$(<"$test_root/ephemeral/qemu.log")" tryubuntu.ssh_access=1
...
run_scenario malformed 1 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:02222:22
...
run_scenario reset-only 0 --reset-storage-only \
  TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:2225:22
...
assert_not_contains "$(<"$test_root/reset-only/stderr")" tryubuntu.ssh_access
grep -aq $'ARM\\x64' "$persistent_root/boot/kernel" || fail "re-staged boot-kit kernel is not the raw Image stub"
assert_contains "$(<"$persistent_root/boot/initramfs")" new-initramfs
assert_contains "$(<"$persistent_root/boot/command-line")" loglevel=5

/usr/bin/plutil -replace kernelCommandLine -string \
  'root=/dev/vda rw rootwait console=tty0 console=hvc0 tryubuntu.ssh_access=0' \
  "$guest/launch.plist"
run_scenario prebaked-token 1 '' TRYUBUNTU_QEMU_GPU_PORT_FORWARDS=tcp:2222:22
