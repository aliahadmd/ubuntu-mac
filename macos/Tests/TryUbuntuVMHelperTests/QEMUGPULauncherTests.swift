...
import Foundation
import Testing
@testable import TryUbuntuVMHelper
...
struct QEMUGPULaunchRequestTests {
    ...
    func parsesAllowedArguments() {
        let guest = "/private/tmp/tryubuntu guest"
    ...
    func validatesGuestDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tryubuntu-qemu-request-\(UUID().uuidString)", isDirectory: true)
...
struct QEMUStandardErrorDrainTests {
    ...
    func parsesReadyControlSocket() {
        let partial = "startup output\n[qemu-gpu] Ready. QMP: /tmp/tryubuntu-qemu-gpu.A1b2C3/qmp"
        ...
        #expect(QEMUGPUProcessSupervisor.virtualMachineReadyEvent(in: complete) ==
            .virtualMachineReady(
                qmpSocketPath: "/tmp/tryubuntu-qemu-gpu.A1b2C3/qmp.sock"
            ))
    }
    ...
    func readyMarkerMustStartItsLine() {
        let output = """
            ...
            [qemu-gpu] Ready. QMP: /tmp/tryubuntu-qemu-gpu.Z9y8X7/qmp.sock
            """ + "\n"
        #expect(QEMUGPUProcessSupervisor.virtualMachineReadyEvent(in: output) ==
            .virtualMachineReady(
                qmpSocketPath: "/tmp/tryubuntu-qemu-gpu.Z9y8X7/qmp.sock"
            ))

        let lookalike = "[qemu-gpu] Ready.bad QMP: /tmp/tryubuntu-qemu-gpu.Z9y8X7/qmp.sock\n"
    ...
    func rejectsMalformedReadyControlSocket() {
        for path in [
            "/tmp/tryubuntu-qemu-gpu.short/qmp.sock",
            "/tmp/tryubuntu-qemu-gpu.A1b2C3/../qmp.sock",
            "/private/tmp/tryubuntu-qemu-gpu.A1b2C3/qmp.sock",
...
struct QEMUGPUStorageSpaceEstimateTests {
    ...
    func displaysStorageRoot() throws {
        let configuredRoot = "/private/tmp/try-tryubuntu-configured/../data"
        let configuredEnvironment = ["TRYUBUNTU_QEMU_GPU_STATE_ROOT": configuredRoot]
        ...
        for invalidRoot in ["relative/path", "/private/tmp/..", "/tmp", "/bad\npath"] {
            let invalidEnvironment = ["TRYUBUNTU_QEMU_GPU_STATE_ROOT": invalidRoot]
            #expect(QEMUGPUStorageSpaceEstimate.dataDirectoryURL(
    ...
    func honorsStoredPreference() throws {
        let container = FileManager.default.temporaryDirectory
            .appendingPathComponent("tryubuntu-launcher-\(UUID().uuidString)", isDirectory: true)
        ...
        // script treats TRYUBUNTU_QEMU_GPU_STATE_ROOT — the picked folder is
    ...
    func overrideBeatsPreference() {
        let configured = "/private/tmp/try-tryubuntu-override"
        let expected = URL(fileURLWithPath: configured, isDirectory: true).standardizedFileURL
        #expect(QEMUGPUStorageSpaceEstimate.storageRootURL(
            environment: ["TRYUBUNTU_QEMU_GPU_STATE_ROOT": configured],
    ...
    func selectsStorageKey() {
        ...
        #expect(QEMUGPUStorageSpaceEstimate.storageKey(
            environment: ["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK": "0"],
            bundleIdentity: identity
        ) == "current")
        #expect(QEMUGPUStorageSpaceEstimate.storageKey(
            environment: ["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK": "1"],
            bundleIdentity: identity
        ) == identity)
        #expect(QEMUGPUStorageSpaceEstimate.storageKey(
            environment: ["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK": "1"],
        ...
        #expect(QEMUGPUStorageSpaceEstimate.storageKey(
            environment: ["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK": "invalid"],
            bundleIdentity: identity
        ) == nil)
    }
    ...
    func countsResettableLegacyDisks() throws {
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "tryubuntu-space-estimate-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let environment = ["TRYUBUNTU_QEMU_GPU_STATE_ROOT": root.appendingPathComponent("state").path]
        ...
        #expect(QEMUGPUStorageSpaceEstimate.reclaimableBytes(
            environment: environment.merging(
                ["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK": "1"],
    ...
    func ignoresNoncanonicalSerializedMetadata() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "tryubuntu-space-estimate-noncanonical-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
        let disks = root.appendingPathComponent("state/disks", isDirectory: true)
    ...
    func ignoresFractionalMetadataNumbers() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "tryubuntu-space-estimate-fractional-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
        let disks = root.appendingPathComponent("state/disks", isDirectory: true)
...
struct QEMUGPULauncherPathTests {
    ...
    private func makeFixture(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tryubuntu-qemu-path-\(UUID().uuidString)", isDirectory: true)
...
struct AccessibilityLaunchDecisionTests {
    @Test("an unavailable grant never blocks Ubuntu startup")
