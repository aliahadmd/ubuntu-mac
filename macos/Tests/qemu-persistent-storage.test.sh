...
if action == "assert-lock-fdset":
    ...
    matches = [
        ...
        if descriptor.get("opaque") == "tryubuntu-persistent-lock"
    ]
...
test_root=$(mktemp -d '/private/tmp/tryubuntu-qemu-storage-test.XXXXXX')
case "$test_root" in
  /private/tmp/tryubuntu-qemu-storage-test.??????) ;;
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=1
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/missing-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
...
assert test ! -e "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/current"
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/zstd-boot-state"
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" '' \
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/single-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=1
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" '' \
  "$source_bytes" "$kernel_a" "$initramfs_a" "$kernel_command_line_a"
...
assert_eq "$single_disk" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/current/rootfs.ext4"
...
assert_eq "$QEMU_SELECTED_KERNEL" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/kernel"
assert_eq "$QEMU_SELECTED_INITRAMFS" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/initramfs"
assert cmp -s "$QEMU_SELECTED_KERNEL" "$kernel_a"
assert cmp -s "$QEMU_SELECTED_INITRAMFS" "$initramfs_a"
assert_eq "$QEMU_SELECTED_KERNEL_COMMAND_LINE" "$kernel_command_line_a"
assert_eq "$QEMU_PERSISTENT_STORAGE_NEEDS_BOOT_RECOVERY" 0
assert test ! -e "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_b"
qemu_persistent_storage_release_lock
...
assert_eq "$QEMU_PERSISTENT_STORAGE_NEEDS_BOOT_RECOVERY" 0
assert test ! -e "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a"
qemu_persistent_storage_release_lock
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/boot-recovery-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
printf 'X' | dd \
  of="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/kernel" \
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/symlink-boot-state"
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" '' \
  "$source_bytes" "$kernel_a" "$initramfs_a" "$kernel_command_line_a"
...
qemu_persistent_storage_release_lock
/bin/rm -f "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/kernel"
ln -s "$kernel_a" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/kernel"
assert_status 1 qemu_persistent_storage_select_existing \
  "$identity_b" "$kernel_b" "$initramfs_b" "$kernel_command_line_b"
assert test -L "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/boot/$identity_a/kernel"
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/orphan-boot-state"
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" '' \
  "$source_bytes" "$kernel_a" "$initramfs_a" "$kernel_command_line_a"
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/schema-one-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
interrupted_old_reset="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/.current.discarded.interrupted"
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/current-plus-legacy-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
qemu_persistent_storage_select \
  reset "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
assert_eq "$QEMU_SELECTED_DISK" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/current/rootfs.ext4"
assert cmp -s "$QEMU_SELECTED_DISK" "$source_disk"
assert test ! -e "$current_plus_legacy_legacy"
assert_eq \
  "$(find "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d '[:space:]')" \
  1
qemu_persistent_storage_release_lock
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/invalid-current-plus-legacy-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/multi-exact-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=1
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
assert_eq "$multi_exact_disk" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/current/rootfs.ext4"
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT="$test_root/multi-newest-state"
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=1
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
...
invalid_legacy="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/$identity_bad"
...
assert_eq "$multi_newest_disk" "$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/current/rootfs.ext4"
...
recognized_stage="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/.${identity_a}.initializing.ABCDEF"
mkdir "$recognized_stage"
chmod 700 "$recognized_stage"
_qps_write_metadata \
  "$recognized_stage/metadata.json" "$identity_a" "$source_sha" "$source_bytes"
unknown_stage="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/.${identity_a}.initializing.FEDCBA"
mkdir "$unknown_stage"
...
newline_stage="$TRYUBUNTU_QEMU_GPU_STATE_ROOT/disks/.${identity_a}.initializing.NLTEST"
mkdir "$newline_stage"
...
unset TRYUBUNTU_QEMU_GPU_STATE_ROOT
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$saved_state_root
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=$saved_multi_disk

# A broad override is rejected before any mutation.
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=/
assert_fails qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$saved_state_root
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$unsupported_root
export TRYUBUNTU_QEMU_GPU_TEST_FS_TYPE=exfat
assert_fails qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
assert_fails qemu_persistent_storage_materialize_source \
  "$identity_compressed" "$compressed_disk" "$compressed_bytes" \
  "$source_sha" "$source_bytes" "$zstd_test"
unset TRYUBUNTU_QEMU_GPU_TEST_FS_TYPE
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$cramped_root
export TRYUBUNTU_QEMU_GPU_TEST_FREE_BYTES=1024
assert_fails qemu_persistent_storage_materialize_source \
  "$identity_compressed" "$compressed_disk" "$compressed_bytes" \
  "$source_sha" "$source_bytes" "$zstd_test"
assert_fails qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
unset TRYUBUNTU_QEMU_GPU_TEST_FREE_BYTES
...
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$cramped_root
saved_cramped_multi_disk=$TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=0
qemu_persistent_storage_select \
  persistent "$identity_a" "$source_disk" "$source_sha" "$source_bytes" ''
assert_eq "$QEMU_SELECTED_DISK" "$cramped_root/disks/current/rootfs.ext4"
qemu_persistent_storage_release_lock
export TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=$saved_cramped_multi_disk
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$saved_state_root
...
printf '%s\n' 'tryubuntu-qemu-storage-root-v2' >"$marker_file"
chmod 600 "$marker_file"
assert_fails _qps_validate_root_marker "$marker_file"
...
printf 'bogus\n' >"$marker_file"
chmod 600 "$marker_file"
saved_marker_state_root=$TRYUBUNTU_QEMU_GPU_STATE_ROOT
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$marker_root
assert_fails _qps_prepare_state_root
export TRYUBUNTU_QEMU_GPU_STATE_ROOT=$saved_marker_state_root
