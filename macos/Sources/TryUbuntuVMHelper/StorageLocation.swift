...
/// workspace root — Ubuntu never creates a folder inside it on the user's
/// behalf. A folder must already be empty, or already be a workspace Ubuntu
...
enum StorageLocationPolicyError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .notEmpty(let path):
            "This folder already has files in it: \(path). Ubuntu only uses an empty folder, so it never mixes its virtual machine with anything else stored there. Choose or create an empty folder instead."
        ...
        case .unsupportedFilesystem(let filesystem, let volume):
            "\(volume) is formatted as \(filesystem.uppercased()). Ubuntu needs an APFS disk: the VM disk grows as you use it, and other formats would claim its full size right away."
        case .insufficientSpace(let volume, let needed, let available):
            "\(volume) has \(StorageLocationPolicy.format(bytes: available)) free. Ubuntu needs at least \(StorageLocationPolicy.format(bytes: needed)) to create the VM."
        }
    }
}
...
struct StorageLocationLaunchConfiguration: Equatable {
    ...
    /// `TRYUBUNTU_QEMU_GPU_STATE_ROOT` is the documented development and test
...
struct StorageLocationMenuState: Equatable {
    ...
    let problem: String?
    let warning: String?
    /// True when TRYUBUNTU_QEMU_GPU_STATE_ROOT decided this location, overriding
