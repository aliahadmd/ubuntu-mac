...
final class VMApplicationController: NSObject, NSApplicationDelegate {
    ...
    private func resolveStorageLocationAvailability() -> StorageAvailability {
        ...
        do {
            ...
            alert.messageText = "Ubuntu\u{2019}s data folder is unavailable"
    ...
    private func prepareForHostSleep() {
        do {
            ...
            fputs(
                "tryubuntu-vm-helper: could not pause the VM before host sleep: \(error.localizedDescription)\n",
                stderr
            )
        }
    }
    ...
    private func resumeAfterHostWake() {
        do {
            ...
            fputs(
                "tryubuntu-vm-helper: could not resume the VM after host wake: \(error.localizedDescription)\n",
                stderr
            )
    ...
    private func handleVolumeUnmount(at volume: URL) {
        ...
        fputs(
            "tryubuntu-vm-helper: the volume holding the Ubuntu VM was unmounted; stopping\n",
            stderr
        )
        lifecycle.requestQuit()
        supervisor.forward(signal: SIGTERM)
        ...
        let alert = NSAlert()
        alert.alertStyle = .critical
        alert.messageText = "The Ubuntu disk was disconnected"
        alert.informativeText = "The drive holding this VM was removed while it was running, so Ubuntu is shutting down. Reconnect the drive before launching again. Removing the drive while the VM is running can damage it."
    ...
    private func failLaunch(_ error: Error) {
        fputs("tryubuntu-vm-helper: \(error.localizedDescription)\n", stderr)
    ...
    private func finish(status: Int32) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            fputs("tryubuntu-vm-helper: forcing exit; the run loop did not stop on its own\n", stderr)
            Darwin.exit(status)
        }
    }
}
