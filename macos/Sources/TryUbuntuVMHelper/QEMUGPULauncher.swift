...
enum QEMUGPURuntimeEnvironment {
    static let inspectOnlyKey = "TRYUBUNTU_QEMU_GPU_INSPECT_ONLY"
    static let dryRunKey = "TRYUBUNTU_QEMU_GPU_DRY_RUN"
    static let bootRecoveryConsentKey = "TRYUBUNTU_QEMU_GPU_ALLOW_BOOT_RECOVERY"
...
enum QEMUGPUStorageSpaceEstimate {
    private static let stateRootEnvironmentKey = "TRYUBUNTU_QEMU_GPU_STATE_ROOT"
    ...
    /// script treats `TRYUBUNTU_QEMU_GPU_STATE_ROOT`. Only the default location
    ...
    static func bootRecoveryPreflight(
        ...
        guard environment["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK"] ?? "0" == "0",
              let bundleIdentity,
    ...
    static func storageKey(
        ...
        switch environment["TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK"] ?? "0" {
        case "0":
            return "current"
        case "1":
...
enum QEMUGPULauncherPath {
    ...
    static func resolve(bundleURL: URL) throws -> URL {
        ...
        guard standardizedBundle.lastPathComponent == appName,
            ...
            throw HelperError.io("QEMU launch is available only from the built Ubuntu app")
        }
...
struct AccessibilityLaunchDecision: Equatable {
    ...
    static func make(for state: AccessibilityAuthorizationState) -> Self {
        ...
        case .unavailable:
            Self(
                allowsLaunch: true,
                warning: "Accessibility is not active yet; Ubuntu will start without Command-to-Super mapping. The mapping becomes available on a later launch after macOS recognizes the grant."
            )
        }
    }
}
...
struct CameraLaunchDecision: Equatable {
    ...
    static func make(for state: CameraAuthorizationState) -> Self {
        ...
        case .notDetermined:
            Self(
                allowsLaunch: true,
                warning: "Camera access was not requested. Ubuntu will continue without the Mac camera until access is enabled."
            )
        }
    }
}
...
final class QEMUGPUProcessSupervisor: @unchecked Sendable {
    ...
    static func qmpSocketPath(inReadyLine line: String) -> String? {
        ...
        let pathPrefix = "/tmp/tryubuntu-qemu-gpu."
