...
private func usage() -> Never {
    fputs("Usage: tryubuntu-vm-helper --run-qemu [--ephemeral | --reset-storage | --reset-storage-only] [GUEST_DIR] | --bridge-command-super QEMU_PID QMP_SOCKET | --bridge-native-audio QEMU_PID SOCKET ROUTE_DIRECTORY | --bridge-native-camera QEMU_PID SOCKET | --bridge-native-clipboard QEMU_PID SOCKET\n", stderr)
    exit(64)
}
...
do {
    if arguments.first == "--bridge-native-audio" {
        ...
        fputs("[audio-bridge] Host audio devices are available inside Ubuntu.\n", stderr)
        try bridge.run()
        exit(0)
    }
    ...
    if arguments.first == "--bridge-native-clipboard" {
        ...
        fputs("[clipboard-bridge] The Mac clipboard is shared with Ubuntu.\n", stderr)
        try bridge.run()
        exit(0)
    }
    ...
    if arguments.first == "--bridge-native-camera" {
        ...
        fputs("[camera-bridge] The Mac camera is available to Ubuntu on demand.\n", stderr)
        try bridge.run()
        exit(0)
    }
