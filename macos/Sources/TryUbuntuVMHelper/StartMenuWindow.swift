...
final class ResetConfirmationPrompt {
    ...
    init(detail: String) {
        ...
        alert.messageText = "Reset Ubuntu to factory settings?"
...
final class StartMenuWindow: NSObject, NSWindowDelegate {
    ...
    func resetDidFinish(errorMessage: String?) {
        ...
        if let errorMessage {
            alert.alertStyle = .critical
            alert.messageText = "Ubuntu couldn’t be reset"
            alert.informativeText = errorMessage
        } else {
            alert.alertStyle = .informational
            alert.messageText = "Ubuntu has been reset"
    ...
    /// "Ubuntu has been reset" would be a lie about a destructive action.
    func resetDidAbort() {
        guard resetInProgress else { return }
        resetInProgress = false
        render()
    ...
    func launchRequiresReset() {
        ...
        alert.messageText = "Reset Ubuntu to continue"
    ...
    private func render() {
        ...
        let reset = NSButton(
            title: resetInProgress ? "Resetting Ubuntu…" : "Reset Ubuntu",
            target: self,
            action: #selector(resetUbuntu)
        )
        ...
        launchButton.setAccessibilityLabel(launchInProgress ? "Launching Ubuntu" : "Launch Ubuntu")
        if launchInProgress {
        ...
        footerTitle.addAttributes(
            [
                .link: URL(string: "https://x.com/aliahadmd1/")!,
                .foregroundColor: NSColor.linkColor,
            ],
            range: footerNSString.range(of: "@aliahadmd1")
        )
        ...
        do {
            if !FileManager.default.fileExists(atPath: storageLocationURL.path) {
                ...
                guard storageLocationStatus().isDefault else {
                    throw NSError(
                        domain: "TryUbuntu.StorageLocation",
                        code: 2,
                        userInfo: [
    ...
    @objc private func beginStorageLocationSelection() {
        ...
        panel.title = "Choose where to keep the Ubuntu VM"
        panel.message = "Ubuntu puts its VM files straight into the folder you choose \u{2014} it does not create a folder inside it. Pick an empty folder, or one Ubuntu already uses. The drive must be APFS."
        ...
        if let problem = validateStorageLocation(url.path) {
            let alert = NSAlert()
            alert.alertStyle = .warning
            alert.messageText = "That folder can\u{2019}t hold the Ubuntu VM"
        ...
        confirmation.messageText = "Keep the Ubuntu VM here?"
        confirmation.informativeText = """
            Ubuntu will use \(destination) from the next launch.
    ...
    @objc private func beginSharedFolderSelection() {
        ...
        permissionWindowRestorer.cancel()
        let panel = NSOpenPanel()
        panel.title = "Choose a folder to share with Ubuntu"
        panel.message = "Ubuntu will be able to read and change everything inside this folder, linked as ~/<folder name>."
        panel.prompt = "Share"
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.canCreateDirectories = true
    ...
    private func confirmReset() {
        ...
        var detail = "This permanently erases everything in this Ubuntu virtual machine, including apps, files, accounts, and settings. This cannot be undone or recovered."
    ...
    @objc private func launchUbuntu() {
        guard !launchInProgress,
              !resetInProgress,
              !microphoneRequestInFlight,
              !cameraRequestInFlight else { return }
