...
enum StartMenuPresentation {
    static let incompatibleWorkspaceDetail = "The saved VM uses a storage or boot format this version can’t use, or its data folder contains multiple saved VMs. Reset Ubuntu to create a compatible VM. Resetting permanently erases everything in the VM."
    ...
    static func microphone(
        ...
        case .authorized:
            StartMenuPermissionPresentation(
                detail: "Apps in Ubuntu can record from your Mac microphone.",
                isGranted: true,
                actionTitle: nil,
                action: nil
            )
    ...
    static func camera(
        ...
        case .authorized:
            StartMenuPermissionPresentation(
                detail: "Apps in Ubuntu can use your Mac camera while they are recording.",
                isGranted: true,
                actionTitle: nil,
                action: nil
            )
        case .notDetermined:
            StartMenuPermissionPresentation(
                detail: "Optional. The camera turns on only while an Ubuntu app uses it.",
                isGranted: false,
                actionTitle: requestInFlight ? "Waiting…" : "Allow…",
                action: .request
            )
        case .denied:
            StartMenuPermissionPresentation(
                detail: "The Mac camera is off inside Ubuntu.",
                isGranted: false,
                actionTitle: "Open Settings",
                action: .openSettings
            )
    ...
    static func sharedFolder(
        ...
        if let problem = state.problem {
            ...
            detail = "Mac folder: \(displayPath). In Ubuntu: \(guestPath)."
            compactDetailLines = [
                "Mac folder: \(displayPath)",
                "In Ubuntu: \(guestPath)",
            ]
        } else if let displayPath = state.displayPath {
            detail = "Mac folder: \(displayPath). In Ubuntu: Off."
            compactDetailLines = [
                "Mac folder: \(displayPath)",
                "In Ubuntu: Off",
            ]
        } else {
            detail = "Optional. Pick a Mac folder to use inside Ubuntu under the same name."
            compactDetailLines = nil
        }
    ...
    static func portForwarding(
        ...
        if mappings.isEmpty {
            return StartMenuPortForwardingPresentation(
                detail: "Optional. Reach services running in Ubuntu at localhost on this Mac.",
                compactDetailLines: nil,
                isGranted: false,
        ...
        if mappings.count == 1, let mapping = mappings.first {
            return StartMenuPortForwardingPresentation(
                detail: "localhost:\(mapping.hostPort) → "
                    + "Ubuntu:\(mapping.guestPort) · \(mapping.protocol.displayName)",
                compactDetailLines: [
                    "Mac: localhost:\(mapping.hostPort)",
                    "Ubuntu: port \(mapping.guestPort) · \(mapping.protocol.displayName)",
                ],
                isGranted: true,
    ...
    static func immersiveDetail(isEnabled: Bool) -> String {
        isEnabled
            ? "Ubuntu opens Full Screen with the Mac menu bar and Dock hidden."
            : "Ubuntu opens in a window with the Mac menu bar and Dock available."
    }
}
