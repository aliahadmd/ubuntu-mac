...
qemu_url="https://gitlab.com/api/v4/projects/qemu-project%2Fqemu/repository/archive.tar.gz?sha=$qemu_commit"
qemu_sha256=bbc9627a6eb0f0c937a205a17ca3ef87761a59415b2c39900c72bddce5858943

texture_patch_sha256=b20bdf9a7d7ccda5b86366ad9d09a3bf95308b98a06b1ece281344405bcc7ab9
gpu_fix_patch_sha256=36d964fbd4bd8780de1a6bfe4f4beb75a77d73297c389b294e4eddf71f9a6637
identity_patch_sha256=c0c647547eb1d48c7b0d54515fa5398befb05024a3babda0ef3fe236f5b3dd27
display_patch_sha256=1ce59350b6b8e6842bc0c9ca34c97f54cb75e85e2d7b35e5b483858654c4d693
immersive_patch_sha256=c77c70323458106f948f6b4a3bff2e60e61b58243d08e6b30c2689b6baefbdef
full_grab_patch_sha256=332e226b0c01f02ea9d18d82961e52e948df2062a9ade3f83d6c0da49e2620be
pause_ownership_patch_sha256=c2b8d104bb98adb353808ed93561bfc8f410959ee3b2c96691ca037509ccedc2
audio_device_patch_sha256=9da5a1fad2d36c6c76e2d1d67a310e7d9353bfa47a826601b7b2bc11390dde37
shared_folder_patch_sha256=66554119a3355dc76b26105892297dd7bd4118b787a21692c9a7f9bbce6bc9fa
...
keycodemap_url="https://gitlab.com/api/v4/projects/qemu-project%2Fkeycodemapdb/repository/archive.tar.gz?sha=$keycodemap_commit"
keycodemap_sha256=b80915dc0b1e2ca9868b7492c422e9acb0b452394614c77c824b4dda284b7ea5
...
# QEMU 11.1's offline mkvenv needs the qmp library wheel beside the build
# tooling; PyPI is the primary source and files.pythonhosted.org serves it.
qmp_archive_name=qemu_qmp-0.0.6-py3-none-any.whl
qmp_url="https://files.pythonhosted.org/packages/47/8e/0d41329b82879ee1d4b522566c6d06368119650f8143168f3c3edf2a8afe/$qmp_archive_name"
qmp_sha256=5d7c5af0e9de427696e3bf72e333965c3a697929f77f6b7ddc30c989fc7b539b

# mkvenv's own bootstrap needs packaging before it can install anything else.
...
remove_work_dir() {
  ...
  [[ $path == /private/tmp/tryubuntu-qemu-source-build.* ]] || \
...
work_dir=$(mktemp -d /private/tmp/tryubuntu-qemu-source-build.XXXXXX)
...
download_and_verify() {
  ...
  # Verified-mirror fallback (plan/contract.md section 4): GitHub release
  # assets are unreachable from some networks this project builds on. The
  # proxy serves the same blob, and the pinned checksum still gates the file.
  if [[ $url == https://github.com/* ]]; then
    curl --fail --location --silent --show-error \
      --proto '=https' --tlsv1.2 --retry 3 --connect-timeout 20 \
      --output "$output" "$url" || {
      rm -f "$output"
      attempt_url="https://ghfast.top/$url"
      log "Primary download failed; retrying $label from the verified proxy"
      curl --fail --location --silent --show-error \
        --proto '=https' --tlsv1.2 --retry 3 --connect-timeout 20 \
        --output "$output" "$attempt_url"
    }
  else
    curl --fail --location --silent --show-error \
      --proto '=https' --tlsv1.2 --retry 3 --connect-timeout 20 \
...
qmp_archive="$archive_dir/$qmp_archive_name"
...
obtain_and_verify "qemu.qmp" "$qmp_url" "$qmp_sha256" "$qmp_archive"

validate_tar_root "QEMU $qemu_commit" "$qemu_archive" "$qemu_root-$qemu_commit" "$listing_dir/qemu.txt"
validate_tar_root "keycodemapdb" "$keycodemap_archive" "$keycodemap_root-$keycodemap_commit" "$listing_dir/keycodemapdb.txt"
...
# The API archive's root carries the commit twice; normalize it to the
# expected checkout directory name.
mv "$source_parent/$qemu_root-$qemu_commit" "$source_parent/$qemu_root"
...
install -m 0644 "$setuptools_archive" "$wheel_archive" "$pip_archive" \
  "$qmp_archive" "$packaging_archive" "$source_dir/python/wheels/"
