...
build_dmg=0
guest_dir=
sign_identity=${TRYUBUNTU_CODESIGN_IDENTITY:--}
notarize_profile=
while (($#)); do
  case "$1" in
...
app_icon_source="$macos_dir/TryUbuntuIcon.svg"
...
iconset="$macos_dir/.build/TryUbuntu.iconset"
generated_icon="$macos_dir/.build/TryUbuntu.icns"
...
install -m 0755 "$helper" "$contents/MacOS/tryubuntu-vm-helper"
install -m 0644 "$macos_dir/Info.plist" "$contents/Info.plist"
install -m 0644 "$generated_icon" "$contents/Resources/TryUbuntu.icns"
...
app_sign_options=("${sign_options[@]}" --identifier dev.tryubuntu.native)
qemu_sign_options=("${sign_options[@]}" --identifier dev.tryubuntu.native)
...
codesign "${app_sign_options[@]}" \
  --entitlements "$macos_dir/tryubuntu-vm-helper.entitlements" \
  "$contents/MacOS/tryubuntu-vm-helper"

launch_record=$(TRYUBUNTU_QEMU_GPU_INSPECT_ONLY=1 \
