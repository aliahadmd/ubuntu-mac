...
QEMU_PERSISTENT_STORAGE_BOOT_ABI_UEFI='qemu-arm64-uefi-direct-v1'
...
_qps_volume_filesystem() {
  ...
  if [[ -n ${TRYUBUNTU_QEMU_GPU_TEST_FS_TYPE:-} ]]; then
    printf '%s\n' "$TRYUBUNTU_QEMU_GPU_TEST_FS_TYPE"
    return 0
  fi
...
_qps_assert_volume_supported() {
  ...
  [[ $qps_filesystem == apfs ]] || {
    _qps_fail "the Ubuntu workspace requires an APFS volume; $qps_root is on $qps_filesystem"
    return 1
  }
}
...
_qps_free_bytes() {
  ...
  if [[ -n ${TRYUBUNTU_QEMU_GPU_TEST_FREE_BYTES:-} ]]; then
    printf '%s\n' "$TRYUBUNTU_QEMU_GPU_TEST_FREE_BYTES"
    return 0
  fi
...
_qps_assert_free_space() {
  ...
  (( qps_available >= qps_required )) || {
    _qps_fail "not enough free space for the Ubuntu VM: $((qps_required / 1024 / 1024)) MiB required, $((qps_available / 1024 / 1024)) MiB available"
    return 1
  }
}
...
_qps_prepare_state_root() {
  ...
  if [[ -n ${TRYUBUNTU_QEMU_GPU_STATE_ROOT:-} ]]; then
    qps_configured_root=$TRYUBUNTU_QEMU_GPU_STATE_ROOT
  else
    [[ -n ${HOME:-} ]] || {
...
_qps_validate_kernel_command_line() {
  ...
  for qps_argument in $qps_line; do
    case "$qps_argument" in
      ...
      tryubuntu.qemu_virgl=*|tryubuntu.shared_folder_name=*|tryubuntu.ssh_access=*|\
      tryubuntu.export_boot=*|omarchy.qemu_virgl=*|omarchy.shared_folder_name=*|\
      tryomarchy.ssh_access=*|tryomarchy.export_boot=*) return 1 ;;
    esac
  done
...
_qps_assert_boot_source_file() {
  ...
  case "$qps_label" in
    *kernel)
      # Both factory kernel formats are legitimate boot-kit sources: the raw
      # arm64 Image (legacy kits, plain direct boot) and the PE32+ EFI-stub
      # image (Ubuntu factory kits, UEFI boot) — plan/contract.md section 2.
      qps_magic=$(/usr/bin/od -An -tc -j 56 -N 4 "$qps_path" | tr -d '[:space:]') || return 1
      if [[ $qps_magic != ARMd ]]; then
        qps_magic=$(head -c 2 "$qps_path" 2>/dev/null) || return 1
        [[ $qps_magic == 'MZ' ]] || {
          _qps_fail "$qps_label is neither an ARM64 Image nor a PE32+ EFI-stub image"
          return 1
        }
      fi
      ;;
...
_qps_boot_abi_for_kernel() {
  ...
  local qps_kernel=$1
  local qps_magic
  # Raw arm64 Images also start with MZ, so classify by the 0x38 magic first.
  qps_magic=$(dd if="$qps_kernel" bs=1 skip=56 count=4 2>/dev/null) || return 1
  if [[ $qps_magic == $'ARM\x64' ]]; then
    printf '%s\n' "$QEMU_PERSISTENT_STORAGE_BOOT_ABI"
    return 0
  fi
  qps_magic=$(head -c 2 "$qps_kernel" 2>/dev/null) || return 1
  if [[ $qps_magic == 'MZ' ]]; then
    printf '%s\n' "$QEMU_PERSISTENT_STORAGE_BOOT_ABI_UEFI"
    return 0
  fi
  return 1
}
...
_qps_write_boot_metadata() {
  local qps_path=$1
  local qps_abi=$2
  local qps_identity=$3
  local qps_command_line_bytes=$4
  local qps_command_line_sha=$5
  local qps_kernel_bytes=$6
  local qps_kernel_sha=$7
  local qps_initramfs_bytes=$8
  local qps_initramfs_sha=$9
...
_qps_validate_boot_kit_directory() {
  ...
  case $QPS_BOOT_ABI in
    "$QEMU_PERSISTENT_STORAGE_BOOT_ABI"|"$QEMU_PERSISTENT_STORAGE_BOOT_ABI_UEFI") ;;
    *)
...
_qps_stage_boot_kit_locked() {
  ...
  local qps_abi=''
  qps_abi=$(_qps_boot_abi_for_kernel "$qps_kernel") || {
    _qps_fail 'source boot-kit kernel is neither a PE32+ EFI-stub image nor an ARM64 Image'
    return 1
  }
  ...
  if ! _qps_write_boot_metadata \
    "$qps_staging/metadata.json" "$qps_abi" "$qps_identity" \
...
_qps_require_compatible_workspace() {
  ...
  if [[ $QPS_METADATA_SCHEMA != "$QEMU_PERSISTENT_STORAGE_SCHEMA" ]]; then
    _qps_incompatible \
      'the saved VM uses an unsupported storage format; use Reset Ubuntu to continue'
    return $?
  fi
...
_qps_migrate_legacy_single_workspace() {
  ...
  if [[ -e $qps_final || -L $qps_final ]]; then
    ...
    if [[ $qps_mode == persistent ]] && \
      ...
      _qps_incompatible \
        'multiple saved VMs were found; use Reset Ubuntu to return to one supported disk'
      return $?
    fi
    return 0
  fi
  ...
  [[ -n $qps_selected ]] || {
    ...
    _qps_fail 'legacy Ubuntu disks were found, but none are safe to migrate or reset'
    return 1
  }
...
_qps_select_persistent_disk() {
  ...
  _qps_prepare_state_root || return 1
  case "${TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK:-0}" in
    0) ;;
    1) qps_storage_key=$qps_identity ;;
    *)
      _qps_fail 'TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK must be 0 or 1'
      return 1
      ;;
  esac
