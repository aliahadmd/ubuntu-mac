...
case ${TRYUBUNTU_QEMU_GPU_INSPECT_ONLY:-0} in
  0)
    ...
    ;;
  1) ;;
  *) fail "TRYUBUNTU_QEMU_GPU_INSPECT_ONLY must be 0 or 1" ;;
esac
...
for marker in \
  ...
  TRYUBUNTU_SDL_INPUT_DEVICE_NAME \
  TRYUBUNTU_SDL_OUTPUT_DEVICE_NAME; do
...
bundle_validation=$(python3 - "$guest_dir" "$qemu_bin" "$resources_dir/runtime" <<'PY'
import hashlib
import json
import subprocess
import sys
...
zstd_bin = Path(sys.argv[2]).parent / "zstd"
runtime_root = Path(sys.argv[3])

# The Ubuntu factory set replaces the Ubuntu artifacts one-for-one; the
# historical file names are retained (plan/contract.md section 3).
expected_artifacts = {
    ...
    "rootfs.ext4": ("guest-rootfs", "application/vnd.ubuntu.ext4"),
...
exact_keys(
    manifest,
    {"artifacts", "build", "distribution", "guest", "kind", "schemaVersion"},
    "guest-manifest.json",
)
if manifest.get("schemaVersion") != 1 or manifest.get("kind") != "tryubuntu-guest-artifacts":
    fail("guest manifest identity is invalid")
manifest_build = exact_keys(manifest.get("build"), {"builtAt", "builderImageDigest", "sourceDateEpoch"}, "guest manifest build")
manifest_distribution = exact_keys(
    manifest.get("distribution"),
    {"architecture", "corePackages", "debootstrapVariant", "fallbackMirror", "kernelMeta", "kernelMetaVersion", "license", "name", "primaryMirror", "release", "suite", "verifiedAt"},
    "guest manifest distribution",
)
core_packages = exact_keys(
    manifest_distribution.get("corePackages"),
    {"gdm3", "gnome-shell", "initramfs-tools", "linux-image-generic", "network-manager", "pipewire", "v4l2loopback-dkms", "wireplumber", "wl-clipboard"},
    "guest manifest core packages",
)
if not all(re.fullmatch(r"[0-9]+[:.][0-9][A-Za-z0-9.+~-]*", str(v)) for v in core_packages.values()):
    fail("core package pins are malformed")
if (
    manifest_distribution.get("name") != "Ubuntu"
    or manifest_distribution.get("release") != "26.04.1 LTS"
    or manifest_distribution.get("suite") != "resolute"
    or manifest_distribution.get("architecture") != "arm64"
):
    fail("guest manifest is not the pinned Ubuntu arm64 factory")
...
if (
    ...
    or manifest_guest.get("distribution") != "Ubuntu"
    or manifest_profile != "factory"
    or manifest_guest.get("username") is not None
):
    fail("guest manifest is not a native ARM64 Ubuntu factory guest")

spec, _ = load_json(guest / "build-spec.json", "build-spec.json")
exact_keys(
    spec,
    {"authenticity", "distribution", "guest", "image", "inputs", "runtime", "schemaVersion"},
    "build-spec.json",
)
...
profile_contract = {
    "filesystemLabel": "tryubuntu",
    "filesystemUuid": "e4b1a7c2-9d3f-4a58-b6e1-2c7d5f0a9b31",
    "sizeMiB": 6144,
    "hostname": "ubuntu-factory",
    "username": None,
    "uid": None,
    "defaultTheme": None,
}
...
if spec.get("distribution") != manifest_distribution:
    fail("manifest and build spec distributions differ")

inputs = exact_keys(spec.get("inputs"), {"packages"}, "build spec inputs")
if inputs.get("packages") != "packages.ubuntu.txt":
    fail("build spec inputs do not match the reviewed Ubuntu package list")
...
boot = exact_keys(
    runtime.get("boot"),
    {"abi", "firmware", "kernelImageFormat"},
    "build spec boot",
)
firmware = exact_keys(
    boot.get("firmware"),
    {"code", "mode", "varsPolicy", "varsTemplate"},
    "build spec boot firmware",
)
if (
    boot.get("abi") != "qemu-arm64-uefi-direct-v1"
    or boot.get("kernelImageFormat") != "pe32plus-efi-stub"
    or firmware.get("mode") != "pflash"
    or firmware.get("varsPolicy") != "ephemeral-per-run"
    or firmware.get("code") != "share/edk2-aarch64-code.fd"
    or firmware.get("varsTemplate") != "share/edk2-arm-vars.fd"
):
    fail("UEFI boot contract is invalid")
for firmware_path in (firmware.get("code"), firmware.get("varsTemplate")):
    candidate = runtime_root / firmware_path
    if not candidate.is_file() or ".." in Path(firmware_path).parts:
        fail(f"bundled UEFI firmware is missing or unsafe: {firmware_path}")
clipboard = {
...
shared_folder = {
    ...
    "guestLinkNameParameter": "tryubuntu.shared_folder_name",
}
graphics = {
...
if (
    ...
    or runtime.get("recommendedMemoryMiB") != 8192
    or runtime.get("minimumCpuCount") != 4
):
    fail("native runtime contract is invalid")

authenticity = exact_keys(
    spec.get("authenticity"),
    {"postBuildUserInstallers", "requiredPaths"},
    "build spec authenticity",
)
if authenticity.get("postBuildUserInstallers") != []:
    fail("factory must not carry post-build installer boundaries")
required_paths = authenticity.get("requiredPaths")
if (
    not isinstance(required_paths, list)
    or required_paths != sorted(required_paths)
    or not all(
        isinstance(path, str) and not path.startswith("/") and ".." not in Path(path).parts
        for path in required_paths
    )
):
    fail("required guest paths are declared as sorted relative paths")
...
if any(argument.startswith("tryubuntu.qemu_virgl=") for argument in arguments):
    fail("kernel command line already contains a QEMU VirGL role")
if any(argument.startswith("tryubuntu.shared_folder_name=") for argument in arguments):
    fail("kernel command line already contains a shared folder name")
if any(argument.startswith("tryubuntu.ssh_access=") for argument in arguments):
    fail("kernel command line contains a launcher-owned SSH activation argument")
if any(argument.startswith("tryubuntu.export_boot=") for argument in arguments):
    fail("kernel command line contains a launcher-owned recovery argument")
...
# The new guest kernel is a PE32+ EFI-stub image booted through the bundled
# UEFI firmware; the legacy raw Image format is never accepted here.
kernel = guest / "vmlinuz-linux"
try:
    with kernel.open("rb") as handle:
        if handle.read(2) != b"MZ":
            fail("vmlinuz-linux is not a PE32+ EFI-stub image")
        handle.seek(60)
        pe_offset_bytes = handle.read(4)
        if len(pe_offset_bytes) != 4:
            fail("vmlinuz-linux has an invalid PE header pointer")
        pe_offset = int.from_bytes(pe_offset_bytes, "little")
        handle.seek(pe_offset)
        if handle.read(4) != b"PE\x00\x00":
            fail("vmlinuz-linux has an invalid PE header")
        if handle.read(2) != b"\x64\xaa":
            fail("vmlinuz-linux is not an ARM64 PE image")
except OSError as error:
    fail(f"cannot inspect vmlinuz-linux: {error}")

with (guest / "initramfs-linux.img").open("rb") as handle:
    initramfs_data = handle.read(1024 * 1024)
if initramfs_data[:4] == b"\x28\xb5\x2f\xfd":
    if not zstd_bin.is_file():
        fail("bundled zstd is missing")
    probe = subprocess.run(
        [str(zstd_bin), "-d", "-q", "-c"],
        input=initramfs_data,
        capture_output=True,
    )
    if probe.returncode != 0:
        fail("initramfs zstd frame is invalid")
    initramfs_data = probe.stdout
elif initramfs_data[:2] == b"\x1f\x8b":
    import gzip

    try:
        initramfs_data = gzip.decompress(initramfs_data)
    except OSError as error:
        fail(f"initramfs gzip stream is invalid: {error}")
if initramfs_data[:6] not in {b"070701", b"070702"}:
    fail("initramfs-linux.img does not decompress to a newc cpio archive")
...
if ! qemu_port_forwarding_configure "${TRYUBUNTU_QEMU_GPU_PORT_FORWARDS:-}"; then
...
if ((QEMU_PORT_FORWARDING_ENABLES_SSH)); then
  ssh_kernel_argument=' tryubuntu.ssh_access=1'
fi
...
shared_folder=${TRYUBUNTU_QEMU_GPU_SHARED_FOLDER:-}
...
if [[ -n $shared_folder ]]; then
  ...
  shared_folder_kernel_argument=" tryubuntu.shared_folder_name=$shared_folder_name_encoded"
fi
...
cleanup() {
  ...
  if [[ -n $work_dir && -n $owner_marker && -n $owner_token ]]; then
    case "$work_dir" in
      /private/tmp/tryubuntu-qemu-gpu.??????)
...
reap_stale_work_dirs() {
  ...
  for candidate in /private/tmp/tryubuntu-qemu-gpu.??????; do
...
recover_persistent_boot_kit() {
  ...
  [[ ${TRYUBUNTU_QEMU_GPU_DRY_RUN:-0} == 0 ]] || {
  ...
  recovery_command_line+=' rootflags=noload fsck.mode=skip tryubuntu.export_boot=1'

  echo '[qemu-gpu] Pairing the saved VM with its original boot files (one time).' >&2
  recovery_uefi_args=()
  bundled_image_magic=$(dd if="$bundled_kernel" bs=1 skip=56 count=4 2>/dev/null) || true
  if [[ $bundled_image_magic != $'ARM\x64' && $(head -c 2 "$bundled_kernel" 2>/dev/null) == 'MZ' ]]; then
    # The current factory boots through the bundled UEFI firmware; recovery
    # uses the same boot method as the launch it prepares.
    for firmware_file in "$uefi_firmware_code" "$uefi_firmware_vars_template"; do
      [[ -f $firmware_file && ! -L $firmware_file ]] || {
        fail "bundled UEFI firmware is missing: $firmware_file"
      }
    done
    cp "$uefi_firmware_vars_template" "$work_dir/efi-vars.fd" || fail "cannot stage UEFI variables"
    chmod 0600 "$work_dir/efi-vars.fd"
    recovery_uefi_args=(
      -drive "if=pflash,format=raw,file=$uefi_firmware_code,readonly=on"
  ...
  "$qemu_bin" \
    ...
    "${recovery_uefi_args[@]+"${recovery_uefi_args[@]}"}" \
    -kernel "$bundled_kernel" \
    -initrd "$bundled_initramfs" \
    -append "$recovery_command_line" \
    -drive "if=none,id=tryubuntu-recovery-root,file=$working_disk,format=raw,media=disk,cache=none,readonly=on" \
    -device 'virtio-blk-pci,drive=tryubuntu-recovery-root,serial=tryubuntu-root' \
    -device 'virtio-serial-pci,id=tryubuntu-recovery-serial' \
    -chardev 'stdio,id=tryubuntu-recovery-hvc0,signal=off' \
    -device 'virtconsole,bus=tryubuntu-recovery-serial.0,nr=0,chardev=tryubuntu-recovery-hvc0' \
    -fsdev "local,id=tryubuntu-boot-export,path=$boot_export_dir,security_model=none,multidevs=remap" \
    -device 'virtio-9p-pci,fsdev=tryubuntu-boot-export,mount_tag=try-omarchy-boot-export,romfile=' \
...
umask 077
work_dir=$(mktemp -d '/private/tmp/tryubuntu-qemu-gpu.XXXXXX') || {
...
case ${TRYUBUNTU_QEMU_GPU_DRY_RUN:-0} in
  0|1) ;;
  *) fail "TRYUBUNTU_QEMU_GPU_DRY_RUN must be 0 or 1" ;;
esac
case ${TRYUBUNTU_QEMU_GPU_ALLOW_BOOT_RECOVERY:-0} in
  0|1) ;;
  *) fail "TRYUBUNTU_QEMU_GPU_ALLOW_BOOT_RECOVERY must be 0 or 1" ;;
esac
if (( QEMU_PERSISTENT_STORAGE_NEEDS_BOOT_RECOVERY )); then
  if [[ ${TRYUBUNTU_QEMU_GPU_ALLOW_BOOT_RECOVERY:-0} != 1 ]]; then
...
# The boot method follows the staged kernel's own format (plan/contract.md
# section 2): new factory kits are PE32+ EFI-stub images that boot through the
# bundled UEFI firmware; legacy raw Image kits keep plain direct boot.
# Raw arm64 Images ALSO begin with MZ (their EFI-stub header), so the
# discriminator is the ARM\x64 magic at offset 0x38: present → legacy direct
# boot; absent with a valid PE header → UEFI boot (plan/contract.md §2).
launch_image_magic=$(dd if="$launch_kernel" bs=1 skip=56 count=4 2>/dev/null) || fail "cannot read the selected kernel"
use_uefi=0
if [[ $launch_image_magic != $'ARM\x64' ]]; then
  launch_kernel_magic=$(head -c 2 "$launch_kernel" 2>/dev/null) || fail "cannot read the selected kernel"
  [[ $launch_kernel_magic == 'MZ' ]] || {
    fail "the selected kernel is neither an ARM64 Image nor a PE32+ EFI-stub image"
  }
  use_uefi=1
  for firmware_file in "$uefi_firmware_code" "$uefi_firmware_vars_template"; do
    [[ -f $firmware_file && ! -L $firmware_file ]] || {
...
case ${TRYUBUNTU_QEMU_GPU_IMMERSIVE:-1} in
  1)
  ...
  0)
    ...
    cocoa_immersive=off
    ;;
  *) fail "TRYUBUNTU_QEMU_GPU_IMMERSIVE must be 0 or 1" ;;
esac
...
qemu_args=(
  ...
  -netdev "$qemu_netdev"
  -device 'virtio-net-pci,netdev=tryubuntu-net,mac=52:54:00:12:34:56,romfile='
  -audiodev 'sdl,id=tryubuntu-audio'
  -device 'intel-hda,id=tryubuntu-hda,romfile='
  -device 'hda-micro,bus=tryubuntu-hda.0,audiodev=tryubuntu-audio'
  -serial none
  -monitor none
  -qmp "unix:$qmp_socket,server=on,wait=off"
)
...
qemu_args+=(
  ...
  -append "$launch_kernel_command_line tryubuntu.qemu_virgl=1$shared_folder_kernel_argument$ssh_kernel_argument"
  -drive "if=none,id=tryubuntu-root,file=$working_disk,format=raw,media=disk,cache=writeback"
  -device 'virtio-blk-pci,drive=tryubuntu-root,serial=tryubuntu-root'
  -device "$gpu_device"
  ...
  -object 'rng-random,id=tryubuntu-rng,filename=/dev/urandom'
  -device 'virtio-rng-pci,rng=tryubuntu-rng'
  -device virtio-balloon-pci
  -device 'virtio-serial-pci,id=tryubuntu-serial'
  -chardev 'stdio,id=tryubuntu-hvc0,signal=off'
  -device 'virtconsole,bus=tryubuntu-serial.0,nr=0,chardev=tryubuntu-hvc0'
  -chardev "socket,id=tryubuntu-audio-bridge,path=$audio_bridge_socket,server=on,wait=off"
  -device 'virtserialport,bus=tryubuntu-serial.0,nr=1,chardev=tryubuntu-audio-bridge,name=dev.tryomarchy.audio'
  -chardev "socket,id=tryubuntu-clipboard-bridge,path=$clipboard_bridge_socket,server=on,wait=off"
  -device 'virtserialport,bus=tryubuntu-serial.0,nr=2,chardev=tryubuntu-clipboard-bridge,name=dev.tryomarchy.clipboard'
  -chardev "socket,id=tryubuntu-camera-bridge,path=$camera_bridge_socket,server=on,wait=off"
  -device 'virtserialport,bus=tryubuntu-serial.0,nr=4,chardev=tryubuntu-camera-bridge,name=dev.tryomarchy.camera'
)
...
if [[ -n $shared_folder ]]; then
  ...
  # The patched local driver reports this user's files as the Ubuntu owner
  # account so the guest kernel grants matching read/write access.
  qemu_args+=(
    -fsdev "local,id=tryubuntu-share,path=$shared_folder,security_model=none,multidevs=remap,guest_owner_uid=$shared_folder_guest_owner_uid,guest_owner_gid=$shared_folder_guest_owner_gid"
    -device "virtio-9p-pci,fsdev=tryubuntu-share,mount_tag=$shared_folder_mount_tag,romfile="
  )
fi
...
# direction-specific Ubuntu variables instead; unset means live System Default.
unset SDL_AUDIO_DEVICE_NAME
export TRYUBUNTU_SDL_AUDIO_CONTROL_DIRECTORY="$audio_route_dir"
...
if [[ ${TRYUBUNTU_QEMU_GPU_DRY_RUN:-0} == 1 ]]; then
