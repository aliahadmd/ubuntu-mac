...
remove_generated_dir() {
  ...
  if [[ $path == /private/tmp/tryubuntu-qemu-gpu-runtime.* || \
...
work_dir=$(mktemp -d /private/tmp/tryubuntu-qemu-gpu-runtime.XXXXXX)
...
while IFS=$'\t' read -r archive_name member destination; do
  ...
  install -d -m 0755 "$staged_runtime/$(dirname "$destination")"
  install -m 0755 "$extract_dir/$member" "$staged_runtime/$destination"
done < <(pinned_runtime_member_manifest)

# The UEFI firmware pair boots the Ubuntu guest (plan/contract.md section 2);
# the blobs arrive with the pinned qemu bottle and are digest-verified by
# verify_runtime_tree below.
edk2_code_digest="47765fe344818cbc464b1c14ae658fb4b854f5c2ceffa982411731eb4865594d"
edk2_vars_digest="b3b855c5a80310168051164986855692d1bdb06e67619856177965cd87c6774f"

runtime_files=()
runtime_file_count=0
while IFS= read -r relative || [[ -n $relative ]]; do
  [[ $relative =~ ^(bin|lib|share)/[A-Za-z0-9][A-Za-z0-9._+-]*$ ]] || \
    [[ $relative == share/edk2-aarch64-code.fd || $relative == share/edk2-arm-vars.fd ]] || \
...
verify_runtime_tree() {
  ...
  printf '%s  %s\n' "$edk2_code_digest" "$root/share/edk2-aarch64-code.fd" | shasum -a 256 -c - >/dev/null || \
    die "published UEFI code firmware digest mismatch"
  printf '%s  %s\n' "$edk2_vars_digest" "$root/share/edk2-arm-vars.fd" | shasum -a 256 -c - >/dev/null || \
    die "published UEFI vars firmware digest mismatch"
