...
and `try-ubuntu-ali` notarytool profile. The app builder is also directly usable
...
macos/build-app.sh \
  ...
  --notarize-profile try-ubuntu-ali
```
...
setting `TRYUBUNTU_QEMU_GPU_DEVELOPMENT_MULTI_DISK=1`; release behavior leaves it
...
upgrades Ubuntu. **Cancel** returns to the menu; **Continue** authorizes only
...
to the launcher as `TRYUBUNTU_QEMU_GPU_STATE_ROOT`. An inherited value of that
